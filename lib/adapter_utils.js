...
async function createMyChannel(adapter, device, channel, name, role) {
	if (adapter.config.msgStateCreationPstation) {
		adapter.log.info('____________________________________________');
		adapter.log.info('create  channel ' + channel + ' as role ' + role);
	}
