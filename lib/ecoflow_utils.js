		...
		if (Array.isArray(msgobj.header)) {
			...
			for (let i = 0; i < msgobj.header.length; i++) {
				...
				*/
					...
					if (deviceType === 20) {
						try {
							let msgobj20_1 = decodeMsg(msgobj.header[i].pdata, 'inverter_heartbeat');
							//adapter.log.debug('inv: '+ JSON.stringify(msgobj1));
							returnobj['inverter_heartbeat'] = msgobj20_1;
						} catch (error) {
							adapter.log.debug('id 1 error at: ' + error);
						}
					} else if (deviceType === 2) {
						try {
							let msgobj2_1 = decodeMsg(msgobj.header[i].pdata, 'plug_heartbeat');
							//adapter.log.debug('plug: '+ JSON.stringify(msgobj1));
							returnobj['plug_heartbeat'] = msgobj2_1;
						} catch (error) {
							adapter.log.debug('id 1 error at: ' + error);
						}
					}
