			...
			for (let i = 0; i < msgobj.header.length; i++) {
				...
				*/
					...
					if (deviceType === 20) {
						...
						try {
							let msgobj1 = decodeMsg(msgobj.header[i].pdata, 'plug_heartbeat');
							//adapter.log.debug('inv: '+ JSON.stringify(msgobj1));
							returnobj['plug_heartbeat'] = msgobj1;
						} catch (error) {
							adapter.log.debug('id 1 error at: ' + error);
						}
					}
					...
					if (deviceType === 20 && adapter.config.msgUnknownMsgPstream) {
						...
						adapter.log.debug(
							'unknown packetType cmdId = ' + packetType + ' unknwon deviceType cmdFunc = ' + deviceType
						);
						adapter.log.debug('pdata: ' + msgobj.header[i].pdata.toString('hex'));
					}
				}
