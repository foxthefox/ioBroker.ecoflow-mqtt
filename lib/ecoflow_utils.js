...
async function storeStreamPayload(adapter, stateDictObj, stateObj, topic, payload, devtype, haenable, logged) {
				...
				switch (channel) {
					...
					case 'BPInfo':
						...
						for (let state in payload[channel]) {
							...
							adapter.log.debug('haupd ' + JSON.stringify(haupd));
							if (Object.keys(haupd).length !== 0) haUpdate.push(haupd);
							adapter.log.debug('haUpdate ' + JSON.stringify(haUpdate));
						}
						break;
	...
	adapter.log.debug('final haUpdate ' + JSON.stringify(haUpdate));
	return haUpdate;
}
...
async function compareUpdate(
	...
	if (stateDictObj[origchannel][state]) {
		if (stateDictObj[origchannel][state]['entity'] !== 'icon') {
			...
			if (!old) {
				if (logged) {
					adapter.log.debug(
						'[Compare] force update ' + '.' + channel + '.' + state + ' ' + old.val + ' -> ' + value
					);
				}
	...
	adapter.log.debug('return compare ' + JSON.stringify(haUpdate));
	return haUpdate;
}
