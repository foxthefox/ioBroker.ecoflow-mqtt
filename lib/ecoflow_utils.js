	...
	try {
		...
		if (msgobj) {
			if (Array.isArray(msgobj.header)) {
				...
				for (let i = 0; i < msgobj.header.length; i++) {
					...
					if (msgobj.header[i].pdata) {
						...
						if (prototyp !== 'undef') {
							...
							writeUnknownMsg(adapter, { device: String(topic), topic: msgtype, cmdFunc: String(cmdFunc), cmdId: String(cmdId), buffer: msgobj.header[i].pdata.toString('hex'), src: src })
						}
					} else {
						if (log === true) {
							adapter.log.debug(
					...
					if (msgobj.header[i].pdata) { //msgtype === 'get_reply' && 
				...
				if (msgtype === 'get_reply') { adapter.quotas[topic] = msgobj; }
				return returnobj;
			}
		}
	} catch (error) {
...
function writeUnknownMsg(adapter, msg) {
	...
	if (!adapter.unknownPBmsg[msg.device][msg.topic][msg.cmdFunc][msg.cmdId]) {
		adapter.unknownPBmsg[msg.device][msg.topic][msg.cmdFunc][msg.cmdId] = { pbmsg: msg.buffer, src: msg.src, dst: msg.dst };
	}
}
									...
									if (haenable) {
										...
										switch (enttype) {
											...
											case 'number':
												...
												if (haenable && stateObj[origchannel][type][state]['role'] && stateObj[origchannel][type][state]['role'] === 'level') {
													havalue = String(value);
													haUpdate = {
														topic: adapter.config.haTopic + '/' + topic + '_' + channel + '/' + state,
														payload: havalue
