...
function pstreamDecode(adapter, payload, usage, topic) {
	...
	try {
		adapter.log.debug('[PROTOBUF decode] ' + topic + ' raw (hex): ' + payload.toString('hex'));
		let msgobj = decodeMsg(buf, 'HeaderMessage');
		if (msgobj) {
			if (Array.isArray(msgobj.header)) {
				let returnobj = {};
				let dbgmsg = {};
				adapter.log.debug('[PROTOBUF decode] ' + topic + ' stream has ' + msgobj.header.length + ' messages');
				for (let i = 0; i < msgobj.header.length; i++) {
					let deviceType = msgobj.header[i].cmdFunc;
					if (deviceType) dbgmsg['cmdFunc'] = deviceType;
					let packetType = msgobj.header[i].cmdId;
					if (packetType) dbgmsg['cmdId'] = packetType;
					let src = msgobj.header[i].src;
					if (src) dbgmsg['src'] = src;
					if (msgobj.header[i].dst) dbgmsg['dst'] = msgobj.header[i].dst;
					const seq = msgobj.header[i].seq;
					let dataLen = msgobj.header[i].dataLen;
					if (dataLen) dbgmsg['dataLen'] = dataLen;
					...
					adapter.log.debug(' [PROTOBUF decode] ' + topic + ' msg#' + i + ' => ' + JSON.stringify(dbgmsg));
					if (msgobj.header[i].pdata) {
						...
						adapter.log.debug(' [PROTOBUF decode] ' + topic + ' msg#' + i + ' => no pdata');
					}
					let prototyp = '';
					if (packetType) {
						switch (packetType) {
						...
						if (prototyp !== 'undef') {
							try {
								if (msgobj.header[i].pdata) {
									...
									adapter.log.debug(
										' [PROTOBUF decode] ' + topic + ' msg#' + i + ' => ' + JSON.stringify(msg)
									);
							...
							if (adapter.config.msgUnknownMsgPstream) {
								...
								adapter.log.debug(
									'[PROTOBUF decode] ' + topic + 'Pstream device: ' + msgobj.header[i].deviceSn
								);
								if (msgobj.header[i].pdata) {
									adapter.log.debug(
										'[PROTOBUF decode] ' +
					...
					case 'JTS1_BP_STA_REPORT':
						...
						for (let i = 0; i < payload[channel]['bpSta'].length; i++) {
							for (let state in payload[channel]['bpSta'][i]) {
								...
								adapter.log.debug('got topic ' + channel + ' ( ' + state + ' )' + val);
								haupd = await compareUpdate(
									adapter,
									stateDictObj,
									stateObj,
...
async function compareUpdate(adapter, stateDictObj, stateObj, haenable, topic, channel, state, val, origchannel) {
	let haUpdate = [];
	if (stateDictObj[origchannel][state]) {
		if (stateDictObj[origchannel][state]['entity'] !== 'icon') {
			let value = convertData(stateDictObj, origchannel, state, val, stateObj);
			//adapter.log.debug('converted value ' + value);
		...
		adapter.log.debug('[Compare] ' + topic + ' not processed ' + channel + ' state: ' + state + ' value ' + val);
	}
	return haUpdate;
}
...
async function storePowerkitPayload(adapter, stateDictObj, stateObj, topic, payload, haenable) {
	let haUpdate = [];
	if (payload) {
		try {
			let payloadobj = null;
...
async function prepareStreamCmd(adapter, serial, streamType, state, value, cmd) {
	if (state === 'latestQuotas') {
		...
		if (streamType === 'pstream600' || streamType === 'pstream800') {
			...
			adapter.log.debug(state + ' ' + value + ' send powerocean ' + JSON.stringify(muster));
