	...
	if (payload) {
		try {
			for (let comportion in payload['params']) {
				...
				let state = dotted[1];
				if (channel === 'latestTimeStamp' || state === null || channel === 'kit') {
					//exceptions
				} else {
					// manipulation of delta2
					...
					if (stateDictObj[channel][state]) {
						...
						adapter.log.debug(
							'not processed ' + channel + ' state: ' + state + ' value ' + payload['params'][comportion]
						);
					}
				}
			}
...
function createSubscribeTopics(userID, type1, id1, type2, id2) {
	...
	if (type1 !== 'none') {
		...
		topics.push('/app/' + userID + '/' + id1 + '/thing/property/get_reply');
	...
	if (type2 !== 'none') {
		...
		topics.push('/app/' + userID + '/' + id2 + '/thing/property/get_reply');
