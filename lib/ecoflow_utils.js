...
// tracks consecutive out-of-range readings per "topic.channel.state" key,
// used to filter transient EcoFlow cloud glitches (e.g. occasional voltage spikes)
// while still letting genuinely sustained out-of-range values through after N occurrences
const outlierCounters = {};
const OUTLIER_CONFIRM_COUNT = 3; // number of consecutive out-of-range readings before accepting the value
...
/**
 * Checks if a newly received numeric value is within the min/max range defined
 * for that datapoint. If it's out of range, it is only accepted after it has been
 * seen OUTLIER_CONFIRM_COUNT times in a row for the same datapoint (to filter
 * transient single-message glitches from the EcoFlow cloud). Otherwise the
 * previous (old) value is returned instead.
 *
 * @param {string} counterKey - unique key per device+channel+state, e.g. `${topic}.${channel}.${state}`
 * @param {number} newValue - the freshly converted numeric value
 * @param {number|undefined} oldValue - the previously stored value (fallback if rejected)
 * @param {number|undefined} min - allowed minimum, if defined in the datapoint dict
 * @param {number|undefined} max - allowed maximum, if defined in the datapoint dict
 * @returns {number} the value to actually write to the state
 */
function applyPlausibilityFilter(counterKey, newValue, oldValue, min, max) {
    const numVal = Number(newValue);
    const outOfRange = (max !== undefined && numVal > max) || (min !== undefined && numVal < min);

    if (!outOfRange) {
        // value is fine -> reset counter and accept it
        if (outlierCounters[counterKey]) {
            delete outlierCounters[counterKey];
        }
        return newValue;
    }
                    ...
                    if (
                        ...
                        if (stateDictObj[channel][state]) {
                            if (stateDictObj[channel][state]['entity'] !== 'icon') {
                                ...
                                // plausibility filter (see applyPlausibilityFilter): guards against
                                // transient out-of-range glitches from the EcoFlow cloud across
                                // ALL device types using this "dotted syntax" station path.
                                if (typeof value === 'number' && stateDictObj[channel][state]['entity'] === 'number') {
                                    const dpType = stateDictObj[channel][state]['entity'];
                                    const dpDef = stateObj?.[channel]?.[dpType]?.[state];
                                    if (dpDef && (dpDef['min'] !== undefined || dpDef['max'] !== undefined)) {
                                        const counterKey = `${topic}.${channelsave}.${state}`;
                                        const oldNumVal = old && old.val !== undefined ? Number(old.val) : undefined;
                                        ...
                                        if (filtered !== value) {
                                            adapter.log.debug(
                                                `[Station] plausibility filter: ${topic}.${channelsave}.${state} kept old value ${filtered} (rejected ${value})`,
                                            );
                                            value = filtered;
                                        }
                                    }
            ...
            if (stateDictObj[origchannel]) {
                if (stateDictObj[origchannel][state]) {
                    if (stateDictObj[origchannel][state]['entity'] !== 'icon') {
                        ...
                        // plausibility filter for numeric datapoints: guards against transient
                        // out-of-range glitches from the EcoFlow cloud (e.g. brief voltage spikes).
                        // A value outside min/max is only accepted once it repeats
                        // OUTLIER_CONFIRM_COUNT times in a row; otherwise the previous value is kept.
                        if (typeof value === 'number' && stateDictObj[origchannel][state]['entity'] === 'number') {
                            const dpType = stateDictObj[origchannel][state]['entity'];
                            const dpDef = stateObj?.[origchannel]?.[dpType]?.[state];
                            if (dpDef && (dpDef['min'] !== undefined || dpDef['max'] !== undefined)) {
                                const counterKey = `${topic}.${channel}.${state}`;
                                const oldNumVal = old && old.val !== undefined ? Number(old.val) : undefined;
                                const filtered = applyPlausibilityFilter(
                                    counterKey,
                                    value,
                                ...
                                if (filtered !== value) {
                                    adapter.log.debug(
                                        `[Compare] plausibility filter: ${topic}.${channel}.${state} kept old value ${filtered} (rejected ${value})`,
                                    );
                                    value = filtered;
                                }
                            }
...
function createSubscribeTopics(userID, devices) {
    ...
    if (devices) {
        for (let key in devices) {
            ...
            //topics.push(`/app/device/property/${key}`); // #456 no longer supported!
