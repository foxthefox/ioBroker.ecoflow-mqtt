				...
				switch (channel) {
					...
					case 'BPInfo':
						...
						for (let state in payload[channel]) {
							...
							adapter.log.debug('haUpd return ' + haupd);
							haUpdate = haUpdate.concat(haupd);
							haUpdate2.push(haupd);
							adapter.log.debug('haUpdate ' + haUpdate);
							adapter.log.debug('haUpdate2 ' + haUpdate2);
						}
						break;
						...
						if (stateDictObj[channel][state]) {
							if (stateDictObj[channel][state]['entity'] !== 'icon') {
								...
								if (!old) {
									...
									if (old.val !== value) {
										if (adapter.config.msgUpdateValue) {
											adapter.log.debug(
												'station update ' +
													'.' +
													channelsave +
...
async function compareUpdate(adapter, stateDictObj, stateObj, haenable, topic, channel, state, val, origchannel) {
	...
	adapter.log.debug('final haUpdate ' + haUpdate);
	return haUpdate;
}
