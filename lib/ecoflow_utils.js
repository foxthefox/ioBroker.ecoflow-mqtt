...
async function prepareSHPtimeTaskObj(adapter, serial, channel, state, value) {
    ...
    if (id !== null) {
        params.cfg.id = parseInt(id.val);
    }
    const index = await adapter.getStateAsync(`${serial}.timeTask.${channel}.param.index`).catch(e => {
        adapter.log.warn(`did not get ${serial}.timeTask.${channel}.param.index ->${e}`);
    });
    if (index !== null) {
        params.cfg.cfgIndex = parseInt(index.val);
    }
    ...
    if (type !== null) {
        params.cfg.comCfg.type = parseInt(index.val);
