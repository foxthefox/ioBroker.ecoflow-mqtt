					...
					if (packetType) {
						switch (packetType) {
							...
							case 136:
								...
								break;
							case 135:
								prototyp = deviceType === 20 ? 'brightness_pack' : 'PowerPack'; // plug -> PowerPack?
								break;
							case 134:
			...
			*/
			for (let channel in payload) {
				...
				if (
					...
					let status = await adapter.getStateAsync(topic + '.info.status');
					if (status && status.val === 'offline') {
						await adapter.setStateAsync(topic + '.info.status', { val: 'online', ack: true });
						adapter.log.info(topic + ' is online with cloud');
					}
					...
					if (status && status.val === 'online') {
						...
						adapter.log.info(topic + ' is OFFLINE with cloud');
					}
				}
			}
		} catch (error) {
...
/*
asnyc function storeSHPpayload(adapter, stateDictObj, stateObj, topic, payload, haenable){
	...
	if(payload.params.watth && payload.params.watth.length === 10){

	}
	if(payload.params.watth && payload.params.watth.length === 2){

	}

	if(payload.params.voltages){
		...
		for (const [ device, value ] of Object.entries(pdevices)) {
			...
			if (devtype === 'pstream600' || devtype === 'pstream800' || devtype === 'plug') {
				...
				if (value) {
					...
					await adapter.setStateAsync(device + '.info.latestQuotas', true, false);
				}
			}
		}
	} catch (error) {
