	...
	try {
		...
		if (Array.isArray(msgobj.header)) {
			...
			for (let i = 0; i < msgobj.header.length; i++) {
				...
				if (packetType == 138) {
					try {
						let msgobj138 = decodeMsg(msgobj.header[i].pdata, 'PowerPack');
						//console.log(JSON.stringify(msgobj138));
						//returnobj['PowerPack2'] = msgobj138
					} catch (error) {
						adapter.log.debug('id 138 error at: ' + error);
					}
				} else if (packetType == 136) {
					try {
						let msgobj136 = decodeMsg(msgobj.header[i].pdata, 'PowerPack');
						//console.log(JSON.stringify(msgobj138));
						//returnobj['PowerPack'] = msgobj136
					} catch (error) {
						adapter.log.debug('id 136 error at: ' + error);
					}
				} else if (packetType == 32) {
					/*
				else if (packetType == 131) {
					// set value
					try {
						let msgobj131 = decodeMsg(msgobj.header[i].pdata, 'time_task_config');
						//console.log(JSON.stringify(msgobj131));
						//returnobj['time_task_config'] = msgobj131
					} catch (error) {
						console.log('id 131 error at: ', error);
					}
				} else if (packetType == 130) {
					// set value
					try {
						console.log('130', msgobj.header[i].pdata);
						let msgobj130 = decodeMsg(msgobj.header[i].pdata, 'permanent_watts_pack');
						//console.log(JSON.stringify(msgobj130));
						//returnobj['permanent_watts_pack'] = msgobj130
					} catch (error) {
						console.log('id 130 error at: ', error);
					}
				} else if (packetType == 129) {
					// set value
					try {
						console.log('129', msgobj.header[i].pdata);
						let msgobj129 = decodeMsg(msgobj.header[i].pdata, 'supply_priority_pack');
						//console.log(JSON.stringify(msgobj129));
						//returnobj['supply_priority_pack'] = msgobj129
					} catch (error) {
						console.log('id 129 error at: ', error);
					}
				} 
				*/
					if (usage === 'noderedchart') {
						try {
							let msgobj32 = decodeMsg(msgobj.header[i].pdata, 'EnergyPack');
					...
					try {
						...
						returnobj['InverterHeartbeat2'] = msgobj4;
					} catch (error) {
					...
					try {
						...
						returnobj['inverter_heartbeat'] = msgobj1;
					} catch (error) {
					...
					if (adapter.config.msgUnknownMsgPstream) {
						adapter.log.debug('unknown packetType cmdId = ' + packetType);
						adapter.log.debug('pdata: ' + JSON.stringify(msgobj.header[i].pdata));
						adapter.log.debug('pdata2: ' + msgobj.header[i].pdata.toString('hex'));
					}
				}
			}
			return returnobj;
...
function createSubscribeTopics(userID, streams, stations) {
	let topics = [];
	if (streams) {
			...
			if (
				...
				if (stationType === 'delta2max') {
					...
					if (state === 'cfgDcChgCurrent') {
						...
						msg['params']['dcChgCfg2'] = curr * 1000;
