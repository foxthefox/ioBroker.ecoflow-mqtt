...
message InverterHeartbeat2 {
	optional int32 X_Unknown_1 = 1; //pv1 off/on?
	optional int32 X_Unknown_2 = 2; //status pv1
	optional int32 X_Unknown_3 = 3; //pv2 off/on?
	optional int32 X_Unknown_4 = 4; //status pv2
	optional int32 X_Unknown_5 = 5;
	optional int32 X_Unknown_6 = 6; //status
	optional int32 upperLimit_7 = 7;
	optional int32 lowerLimit_8 = 8;
	optional int32 X_Unknown_9 = 9; //status 48->4096
	optional int32 X_Unknown_10 = 10; //status
	optional int32 X_Unknown_11 = 11;
	optional int32 baseLoad_12 = 12;
	optional int32 X_Unknown_13 = 13; //power to plugs positive
	...
	optional int32 X_Unknown_46 = 46; //power to plugs negative
	...
	optional int32 X_Unknown_52 = 52; //wifiRssi
 }
...
message plug_heartbeat {
	...
	optional int32 stream_power_33 = 33; //total streamoutputpower
	optional int32 sum_incl_plugs_34 = 34; //evtl total all plugs
	...
	optional int32 wifi_rssi_40 = 40;
  }
					...
					if (packetType) {
						...
						if (packetType === 32) {
							if (usage === 'noderedchart') {
								...
								if (msgobj32.sysEnergyStream.sysSeq === 65535) {
	...
	try {
		for (const [ device, value ] of Object.entries(pdevices)) {
			...
			if (devtype === 'pstream600' || devtype === 'pstream800' || devtype === 'plug') {
				adapter.log.debug(device + ' -> latestQuotas requested');
				const value = await adapter.getStateAsync(device + '.info.latestQuotas');
				await adapter.setStateAsync(device + '.info.latestQuotas', value.val === true ? false : true, false);
			}
		}
	} catch (error) {
		adapter.log.debug('getLastQuotas ' + error);
	}
}
...
function prepareStreamCmd(adapter, serial, streamType, state, value, cmd) {
	if (state === 'latestQuotas') {
		adapter.log.debug('latestQuotas');
		let muster = {
			header: {
				src: 32,
				dest: 32,
