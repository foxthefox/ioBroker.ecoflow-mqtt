                    ...
                    if (protoMsg && protoSourceDevice) {
                        if (cmdId) {
                            if (Object.prototype.hasOwnProperty.call(protoMsg['cmdId'], cmdId)) {
                                ...
                                //no decodei possible;
                                if (log === true) {
                                    adapter.log.debug(`[PROTOBUF decode] ${topic} [${msgtype}] msg#${i} unknown cmdId`);
                                }
                            }
                        } else {
                            prototyp = 'undef';
                        ...
                        //no objects
                        adapter.log.debug(
                            `[PROTOBUF decode] ${topic} [${msgtype}] protoMsg ${protoMsg}protoSourceDevice ${
                                protoSourceDevice
            ...
            if (stateDictObj[origchannel]) {
                if (stateDictObj[origchannel][state]) {
                    if (stateDictObj[origchannel][state]['entity'] !== 'icon') {
                        ...
                        if (!old) {
                            if (logged) {
                                adapter.log.debug(`[Compare] force update ` + `.${channel}.${state} -> ${value}`);
                            }
                            ...
                            if (Object.prototype.hasOwnProperty.call(old, 'val')) {
                                if (old.val !== value) {
                                    if (logged) {
                                        adapter.log.debug(
                                            `[Compare] update ` + `.${channel}.${state} ${old.val} -> ${value}`,
                                        );
                                    }
                                    await adapter.setStateAsync(`${topic}.${channel}.${state}`, {
                                        val: value,
            ...
            if (
                ...
                switch (state) {
                    ...
                    case 'chChargeWatt':
                        {
                            delete msg['params'];
                            const params = await prepareSHPtimeTaskObj(adapter, serial, channel, state, value);
                            msg['params'] = params;
                        }
                        break;
                    default:
 ...
 * @param adapter - THIS transfer
 * @param {string} serial - serial#
 * @param {string} channel - timetask#
 * @param {string} state - datapoint
 * @param {number | boolean} value - value
 */
async function prepareSHPtimeTaskObj(adapter, serial, channel, state, value) {
    let params = {
        cfg: {
            param: {
                lowBattery: 95,
            ...
            comCfg: {
                ...
                timeRange: {
                    isCfg: 1,
                    timeMode: 0,
                    startTime: { sec: 0, min: 0, week: 2, hour: 0, month: 10, year: 2022, day: 24 },
                    endTime: { sec: 59, min: 59, week: 2, hour: 23, month: 10, year: 2022, day: 25 },
                    isEnable: 1,
                },
                isEnable: 1,
                setTime: { sec: 61, min: 9, week: 7, hour: 16, month: 11, year: 2022, day: 15 },
            },
        },
        cfgIndex: 1,
        cmdSet: 11,
    ...
    const id = await adapter.getStateAsync(`${serial}.timeTask.${channel}.param.id`).catch(e => {
        adapter.log.warn(`did not get ${serial}.timeTask.${channel}.param.id ->${e}`);
    });
    if (id !== null) {
        params.cfg.id = id.val;
    }
    const cfgIndex = await adapter.getStateAsync(`${serial}.timeTask.${channel}.param.cfgIndex`).catch(e => {
        adapter.log.warn(`did not get ${serial}.timeTask.${channel}.param.cfgIndex ->${e}`);
    });
    if (cfgIndex !== null) {
        params.cfg.cfgIndex = cfgIndex.val;
    }
    const type = await adapter.getStateAsync(`${serial}.timeTask.${channel}.comCfg.type`).catch(e => {
        adapter.log.warn(`did not get ${serial}.timeTask.${channel}.comCfg.type ->${e}`);
    });
    // task object is different when charging or discharging
    if (type == 1) {
        for (let key in params['cfg']['param']) {
            if (key !== 'chChargeWatt') {
                const val = await adapter.getStateAsync(`${serial}.timeTask.${channel}.param.${key}`).catch(e => {
                    adapter.log.warn(`did not get ${serial}.timeTask.${channel}.param.${key} ->${e}`);
                });
                if (val !== null) {
                    //chSta JSON.parse
                    params.cfg.param[key] = val.val;
                }
            }
        }
        delete params.cfg['chSta'];
    } else {
        const cha = await adapter.getStateAsync(`${serial}.timeTask.${channel}.param.chSta`).catch(e => {
            adapter.log.warn(`did not get ${serial}.timeTask.${channel}.param.chSta ->${e}`);
        });
        if (cha !== null) {
            //JSON.parse
            params.cfg.chSta = cha.val;
        }
        ...
        if (key === 'setTime') {
            ...
            for (let key2 in params['cfg']['comCfg'][key]) {
                const val = await adapter
                    ...
                    .catch(e => {
                        adapter.log.warn(`did not get ${serial}.timeTask.${channel}.comCfg.${key}.${key2} ->${e}`);
                    });
                if (val !== null) {
            ...
            for (let key2 in params['cfg']['comCfg'][key]) {
                if (key2 === 'startTime' || key2 === 'endTime') {
                    for (let key3 in params['cfg']['comCfg'][key][key2]) {
                        const val = await adapter
                            .getStateAsync(`${serial}.timeTask.${channel}.comCfg.${key}.${key2}.${key3}`)
                            .catch(e => {
                                adapter.log.warn(
                        ...
                        if (val !== null) {
                            params.cfg.comCfg[key][key2][key3] = val.val;
                        }
                    }
                } else {
                    const val = await adapter
            ...
            const val = await adapter.getStateAsync(`${serial}.timeTask.${channel}.comCfg.${key}`).catch(e => {
                adapter.log.warn(`did not get ${serial}.timeTask.${channel}.comCfg.${key} ->${e}`);
            });
            if (val !== null) {
                //timescale JSON.parse
                params.cfg.comCfg[key] = val.val;
            }
        }
    }
    if (type === 1 && state == 'isEnable') {
        const power = await adapter.getStateAsync(`${serial}.timeTask.${channel}.param.chChargeWatt`).catch(e => {
            adapter.log.warn(`did not get ${serial}.timeTask.${channel}.param.chChargeWatt ->${e}`);
        });
        if (power !== null) {
            params.cfg.param.chChargeWatt = power.val;
        }
        params.cfg.comCfg.isEnable = value ? 1 : 0;
    } else if (type === 1 && state === 'chChargeWatt') {
        const isenabled = await adapter.getStateAsync(`${serial}.timeTask.${channel}.comCfg.isEnable`).catch(e => {
            adapter.log.warn(`did not get ${serial}.timeTask.${channel}.comCfg.isEnable ->${e}`);
        });
        if (isenabled !== null) {
            params.cfg.comCfg.isEnable = isenabled.val ? 1 : 0;
        }
        params.cfg.param.chChargeWatt = parseInt(value);
    } else if (type === 2 && state == 'isEnable') {
        params.cfg.comCfg.isEnable = value ? 1 : 0;
    }
    return params;
}
