			...
			if (link2.entity !== 'array' && link2.entity !== 'icon' && key !== 'info') {
                ...
                */
				...
				if (bat1 && key === 'bmsMaster') {
					...
					//always
					if (value.name) msg1['name'] = value.name;

					//sensor/number/
					if (value.unit_of_measurement) msg1['unit_of_measurement'] = value.unit_of_measurement;
					if (value.device_class) msg1['device_class'] = value.device_class;

					// sensor
					if (value.entity_type === 'sensor') {
						if (value.unit_of_measurement !== 'Wh' && value.unit_of_measurement !== 'kWh') {
							msg1['state_class'] = 'measurement';
							msg1['value_template'] = '{{value|round(3)}}';
						} else {
							msg1['state_class'] = 'total_increasing';
						}
					}
					//binary_sensor
					if (value.payload_on) msg1['payload_on'] = value.payload_on;
					if (value.payload_off) msg1['payload_off'] = value.payload_off;

					// when diagnostic / config
					if (value.entity_category) msg1['entity_category'] = value.entity_category;
				...
				if (bat2 && key === 'bmsMaster') {
					...
					//always
					if (value.name) msg2['name'] = value.name;

					//sensor/number/
					if (value.unit_of_measurement) msg2['unit_of_measurement'] = value.unit_of_measurement;
					if (value.device_class) msg2['device_class'] = value.device_class;

					// sensor
					if (value.entity_type === 'sensor') {
						if (value.unit_of_measurement !== 'Wh' && value.unit_of_measurement !== 'kWh') {
							msg2['state_class'] = 'measurement';
							msg2['value_template'] = '{{value|round(3)}}';
						} else {
							msg2['state_class'] = 'total_increasing';
						}
					}
					//binary_sensor
					if (value.payload_on) msg2['payload_on'] = value.payload_on;
					if (value.payload_off) msg2['payload_off'] = value.payload_off;

					// when diagnostic / config
					if (value.entity_category) msg2['entity_category'] = value.entity_category;
