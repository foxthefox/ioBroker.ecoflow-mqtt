			...
			if (link2.entity !== 'array' && link2.entity !== 'icon' && key !== 'info') {
				switch (key) {
					...
					case 'BMSHeartBeatReport1':
						if (bat1) {
							const msg = prepareDiscoveryMsg(devicestates, key, link2, key2, errors, deviceID, type, topic)
							msgqeue.push(msg);
						}
						break;
					case 'bmsSlave2':
...
function prepareDiscoveryMsg(devicestates, key, link2, key2, errors, deviceID, type, topic) {
	const value = devicestates[key][link2.entity][key2];
	let msg = {};
	if (!value.entity_type) {
		errors.push(key + '_' + key2);
...
function prepareFullHaUpd(deviceID, devicedict, devicestates, topic, bat1, bat2, bat3) {
	...
	Object.entries(devicedict).forEach(([key, link]) => {
		...
		Object.entries(link).forEach(([key2, link2]) => {
			...
			// link ist object mit entity verweis auf devicestates
			if (link2.entity !== 'array' && link2.entity !== 'icon' && key !== 'info' &&
				key !== 'bmsSlave1' && key !== 'bp2' && key !== 'statusReportBattery2' && key !== 'BPInfo1' && key !== 'BMSHeartBeatReport1' &&
				key !== 'bmsSlave2' && key !== 'bp3' && key !== 'statusReportBattery3' && key !== 'BPInfo2' && key !== 'BMSHeartBeatReport2' &&
				key !== 'BMSHeartBeatReport3' && key !== 'BPInfo3') {
				const value = devicestates[key][link2.entity][key2];
				let msg = {
					topic: '',
					getId: '',
			...
			else if (link2.entity !== 'array' && link2.entity !== 'icon' && key !== 'info' &&
				(key === 'bmsSlave1' || key === 'bp2' || key === 'statusReportBattery2' || key === 'BPInfo1' || key === 'BMSHeartBeatReport1') && bat1 ||
				(key === 'bmsSlave2' || key === 'bp3' || key === 'statusReportBattery3' || key === 'BPInfo2' || key === 'BMSHeartBeatReport2') && bat2 ||
