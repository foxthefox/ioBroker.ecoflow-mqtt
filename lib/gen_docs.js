function createobj(device) {
	...
	if (
		...
		states = require('./ef_dpu_data.js').deltaproultraStates;
		dict = require('./ef_dpu_data.js').deltaproultraStatesDict[device];
		ranges = require('./ef_dpu_data.js').deltaproultraRanges[device];
		cmd = require('./ef_dpu_data.js').deltaproultraCmd[device];
	} else if (device === 'alternator') {
		console.log('alternator');
		states = require('./ef_alternator_data.js').alternatorStates;
		dict = require('./ef_alternator_data.js').alternatorStatesDict[device];
		ranges = require('./ef_alternator_data.js').alternatorRanges[device];
		cmd = require('./ef_alternator_data.js').alternatorCmd[device];
	} else if (device === 'plug') {
		console.log('plug');
		states = require('./ef_plug_data.js').plugStates;
		dict = require('./ef_plug_data.js').plugStatesDict[device];
		ranges = require('./ef_plug_data.js').plugRanges[device];
		cmd = require('./ef_plug_data.js').plugCmd[device];
	} else {
		console.log('pstream');
		states = require('./ef_pstream_data.js').pstreamStates;
		ranges = require('./ef_pstream_data.js').pstreamRanges[device];
		cmd = require('./ef_pstream_data.js').pstreamCmd[device];
		if (device === 'pstream600' || device === 'pstream800') {
			device = 'pstream';
		}
		dict = require('./ef_pstream_data.js').pstreamStatesDict[device];
