                    ...
                    for (let channel in payload) {
                        ...
                        switch (channel) {
                            ...
                            case 'ParallelEnergyStreamDetail':
                                {
                                    await setOnlineStatus(adapter, topic);
                                    const system1 = await adapter
                                        .getStateAsync(`${topic}.ParallelEnergyStreamReport.system1_devSn`)
                                        .catch(e => {
                                            adapter.log.warn(
                                                `did not get ParallelEnergyStreamReport.system1_devSn ->${e}`,
                                            );
                                        });
                                    const ocean = await adapter
                                        .getStateAsync(`${topic}.ParallelEnergyStreamReport.ocean_devSn`)
                                        .catch(e => {
                                            adapter.log.warn(
                                                `did not get ParallelEnergyStreamReport.system1_devSn ->${e}`,
                                            );
                                    ...
                                    if (system1 && ocean) {
                                        for (let i = 0; i < payload[channel]['paraEnergyStreamDetail'].length; i++) {
                                            for (let state in payload[channel]['paraEnergyStreamDetail'][i]) {
                                                if (state !== 'timestamp') {
                                                    ...
                                                    let prefix = '';
                                                    if (
                                                        payload[channel]['paraEnergyStreamDetail'][i]['devSn'] === ocean
                                                    ) {
                                                        prefix = 'ocean_';
                                                    }
                                                    if (
                                                        payload[channel]['paraEnergyStreamDetail'][i]['devSn'] ===
                                                        system1
                                                    ) {
                                                        prefix = 'system1_';
                                                    }
                                        ...
                                        adapter.log.debug(
                                            'ParallelEnergyStreamReport did not get system1 or ocean devSn',
                                        );
                                    }
                                }
                                break;
