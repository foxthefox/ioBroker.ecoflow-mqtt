...
const protoMsg = {
    cmdFunc: {
        20: {
            cmdId: {
                1: 'inverter_heartbeat',
                2: 'time_task_config_post',
                //4: 'InverterHeartbeat2',
                129: 'permanent_watts_pack',
                130: 'supply_priority_pack',
                131: 'time_task_config',
                132: 'bat_lower_pack',
                133: 'bat_upper_pack',
                134: 'time_task_config_post',
                135: 'brightness_pack',
                136: 'PowerPack',
                138: 'PowerPack',
                143: 'feed_priority_pack',
                146: 'rated_power_pack',
            },
        },
        32: { cmdId: { 11: 'country_town_message' } },
        254: { cmdId: { 32: 'EnergyPack' } },
    },
};

const msgNameObj = {
    inverter_heartbeat: { cmdId: 1, cmdFunc: 20 },
};
                    ...
                    for (let channel in payload) {
                        ...
                        switch (channel) {
                                                ...
                                                if (range === 'startTime' || range === 'stopTime') {
                                                    for (let time in payload[channel][task][item][range]) {
                                                        await adapter.setStateAsync(
                                                            `${topic}.${channel}.${task}.${item}.${range}.${time}`,
                                                            {
                            ...
                            case 'EnergyPack':
                                ...
                                break;
                            // set and set_reply
                            case 'rated_power_pack':
                            case 'feed_priority_pack':
                            case 'brightness_pack':
                            case 'bat_upper_pack':
                            case 'bat_lower_pack':
                            case 'supply_priority_pack':
                            case 'permanent_watts_pack':
                                for (let state in payload[channel]) {
                                    let val;
                                    let haupd;
                                    val = payload[channel][state];
...
message inverter_heartbeat {
    ...
    optional uint32 permanentWatts = 48; //permanent_watts
    optional uint32 dynamicWatts = 49;
    optional uint32 supplyPriority = 50; //supply_priority
    optional uint32 lowerLimit = 51; //lower_limit
    optional uint32 upperLimit = 52; //upper_limit
...
{
    optional uint32 feedPriority = 1;
}
...
message brightness_pack {
  optional int32 invBrightness = 1;
}
...
message sentinverter_heartbeat {
    setHeader3 header = 1;
}

 message setHeader3 {
    inverter_heartbeat pdata = 1;
    int32 src = 2;
    int32 dest = 3;
    int32 d_src = 4;
    int32 d_dest = 5;
