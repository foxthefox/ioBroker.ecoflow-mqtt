    ...
    DisplayPropertyUpload: {
        diagnostic: {
            ...
            powConsumptionMeasurement: {
                entity_type: 'text',
                entity_category: 'diagnostic',
                name: 'pow Consumption Measurement',
                role: 'info',
                powConsumptionMeasurement: { 0: 'none?', 1: 'plug?', 2: 'meter?' },
            },
            devCtrlStatus: {
                entity_type: 'text',
                entity_category: 'diagnostic',
...
const deviceStatesDict = {
    stream_ac_pro: {
        ...
        DisplayPropertyUpload: {
            ...
            powConsumptionMeasurement: { entity: 'diagnostic' },
        ...
        /*
        setDp3: {
            cfgFeedGridMode: { entity: 'switch' },
            cfgPowConsumptionMeasurement: { entity: 'level' },
            cfgMaxChgSoc: { entity: 'level' },
            cfgMinDsgSoc: { entity: 'level' },
            cfgBackupReverseSoc: { entity: 'level' },
            cfgFeedGridModePowLimit: { entity: 'level' },
            cfgBrightness: { entity: 'level' },
        },
        */
        action: {
        ...
        DisplayPropertyUpload: {
            relay2Onoff: {
                msg: {
                    ...
                    version: 3,
                    payloadVer: 1,
                    pdata: { cfg_utc_time: 6, relay2Onoff: 380 },
                },
            },
            relay3Onoff: {
                msg: {
                    ...
                    version: 3,
                    payloadVer: 1,
                    pdata: { cfg_utc_time: 6, relay3Onoff: 380 },
                },
            },
            ...
            plugInInfoAcInChgPowMax: {
                msg: { dest: 2, cmdFunc: 254, cmdId: 17, dataLen: 7 },
            },
        },
        ConfigWrite: {
            ...
            if (stateObj) {
                for (let channel in payload) {
                    ...
                    switch (channel) {
                        ...
                        case 'ConfigWrite':
                            for (let state in payload[channel]) {
                                if (state !== 'cfgAcOutOpen' && state !== 'cfgDc12vOutOpen' && state !== 'cfgUsbOpen') {
                                    let val;
                                    let haupd;
                                    val = payload[channel][state];
                                    haupd = await compareUpdate(
...
async function prepareProtoCmd(adapter, serial, streamType, state, value, cmd, log) {
    ...
    switch (state) {
        ...
        case 'relay3Onoff':
            muster['header']['pdata'] = {};
            muster['header']['pdata']['cfg_utc_time'] = timestamp;
...
message ConfigWrite {
  ...
  optional uint32 cfg_max_chg_soc = 33; //ac
  optional uint32 cfg_min_dsg_soc = 34; //ac
  optional uint32 cfg_backup_reverse_soc = 102; //ac
  optional uint32 cfg_feed_grid_mode_pow_limit = 169; //ac
  optional bool relay2Onoff = 380; //ac
  optional bool relay3Onoff = 381; //ac
