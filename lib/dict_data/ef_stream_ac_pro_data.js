...
const deviceStatesDict = {
    stream_ac_pro: {
        ...
        DisplayPropertyUpload: {
            ...
            relay2Onoff: { entity: 'diagnostic' },
            relay3Onoff: { entity: 'diagnostic' },
...
async function storeProtoPayload(
    adapter,
    stateDictObj,
    stateObj,
    topic,
                ...
                for (let channel in payload) {
                    ...
                    switch (channel) {
                            ...
                            for (let state in payload[channel]) {
                                ...
                                switch (state) {
                                    ...
                                    case 'runtimeStatisticsSum':
                                        adapter.log.debug('StatisticsSum is available for delta3plus. Please inform developer.')
                                        /*
                        ...
                        case 'setReply_dp3':
                            ...
                            for (let state in payload[channel]) {
                                if (
                                    payload[channel]['configOk'] &&
                                    payload[channel]['configOk'] == true
                                ) {
                                    if (
                                        state !== 'actionId' &&
                                        state !== 'configOk' &&
...
async function prepareProtoCmd(
    adapter,
    serial,
    streamType,
    state,
...
message DisplayPropertyUpload {
   ...
   optional bool relay2_onoff = 980; //bool
   optional bool relay4_onoff = 981; //bool
   optional bool relay3_onoff = 982; //bool
   optional bool relay1_onoff = 983; //bool
   optional uint32 system_group_id = 984;
   optional int32 pow_sys_ac_out_max = 985;
   optional bool plug_in_info_pv3_flag = 987; //bool
   optional bool plug_in_info_pv4_flag = 988; //bool
