...
const deviceStatesDict = {
    poweroceanplus: {
        ...
        JTS1_EMS_HEARTBEAT: {
            ...
            //loadinfo
            pcsLoadInfo_vol: { entity: 'number' },
            pcsLoadInfo_amp: { entity: 'number' },
            pcsLoadInfo_freq: { entity: 'number' },
            pcsLoadInfo_pwr: { entity: 'number' },
            //meterheartbeat
        ...
        JTS1_EMS_CHANGE_REPORT: {
            //reduced, but dict is still there
            emsWorkMode: { entity: 'diagnostic' },
...
const protoMsg = {
    cmdFunc: {
        ...
        50: { cmdId: { 98: 'AppRuquestBpEuLawDataAck' } }, // APP_CMD_ID_EU_LAW_ACK 		AppRuquestBpEuLawData src/dst unterschiedlich
        ...
        96: {
            cmdId: {
                ...
                11: 'EmsEcologyDevReport', // JTS1_ECOLOGY_DEV_REPORT
        ...
        212: {
            cmdId: {
                ...
                99: 'HeatingRodParamSet',
            },
        },
        224: { cmdId: { 1: 'EcologyDevBindListReport', 37: 'id37func224' } },
        //240: { cmdId: { 2: 'EDevBindListReport?' } },
        //241: { cmdId: { 1: 'id1func241', 5: 'id5func241', 36: '' } },
        254: { cmdId: { 32: 'EnergyTotalReport' } }, //->todo
    },
};
                    ...
                    for (let channel in payload) {
                        ...
                        switch (channel) {
                            ...
                            case 'HRChargingParamReport':
                                ...
                                // only when JTS1_EMS_PARAM_CHANGE_REPORT, artificial set value to 0, if datapoint is missing in telegram
                                if (channel === 'JTS1_ENERGY_STREAM_REPORT' && !('mpptPwr' in payload[channel]) && adapter.specialPoweroceanZero) {
                                    let haupd;
                                    haupd = await compareUpdate(
                                        adapter,
                                        stateDictObj,
                                    ...
                                    adapter.log.debug(
                                        `${topic} JTS1_ENERGY_STREAM_REPORT` + ` artificial set mpptPwr = 0`,
                                    );
                                    if (Object.keys(haupd).length !== 0) {
                                        haUpdate.push(haupd);
                                    }
                                }
                                if (channel === 'JTS1_ENERGY_STREAM_REPORT' && !('sysGridPwr' in payload[channel]) && adapter.specialPoweroceanZero) {
                                    let haupd;
                                    haupd = await compareUpdate(
                                        adapter,
                                        stateDictObj,
                                    ...
                                    adapter.log.debug(
                                        `${topic} JTS1_ENERGY_STREAM_REPORT` + ` artificial set sysGridPwr = 0`,
                                    );
                                    if (Object.keys(haupd).length !== 0) {
                                        haUpdate.push(haupd);
                                    }
                                }
                                if (channel === 'JTS1_ENERGY_STREAM_REPORT' && !('bpPwr' in payload[channel]) && adapter.specialPoweroceanZero) {
                                    let haupd;
                                    haupd = await compareUpdate(
                                        adapter,
                                        stateDictObj,
                                    ...
                                    adapter.log.debug(
                                        `${topic} JTS1_ENERGY_STREAM_REPORT` + ` artificial set bpPwr = 0`,
                                    );
                                    if (Object.keys(haupd).length !== 0) {
                                        haUpdate.push(haupd);
                                    }
                                ...
                                break;
                            case 'EnergyStreamDetail':
                                await setOnlineStatus(adapter, topic);
                                if(adapter.specialPoweroceanReportFromDetail){
                                    for (let i = 0; i < payload[channel]['sysEnergyStream'].length; i++) {
                                        for (let state in payload[channel]['sysEnergyStream'][i]) {
                                            let val;
                                            let haupd;
                                            if (state !== 'timezone' && state !== 'timestamp') {
                                                val = payload[channel]['sysEnergyStream'][i][state];
                                                haupd = await compareUpdate(
                                                    adapter,
                                                    stateDictObj,
                                                    stateObj,
                                                ...
                                                adapter.log.debug(
                                                    `${topic} JTS1_ENERGY_STREAM_REPORT` +
                                                        ` artificial set ${state} with ${val} from EnergyStreamDetail`,
                                                );
                                                if (Object.keys(haupd).length !== 0) {
                                                    haUpdate.push(haupd);
                                                }
                                        ...
                                        // artificial set value to 0, if datapoint is missing in telegram
                                        if (!('mpptPwr' in payload[channel]['sysEnergyStream'][i]) && adapter.specialPoweroceanZero) {
                                            let haupd;
                                            haupd = await compareUpdate(
                                                adapter,
                                                stateDictObj,
                                            ...
                                            adapter.log.debug(
                                                `${topic} JTS1_ENERGY_STREAM_REPORT` +
                                                    ` artificial set mpptPwr = 0 from EnergyStreamDetail`,
                                            );
                                            if (Object.keys(haupd).length !== 0) {
                                                haUpdate.push(haupd);
                                            }
                                        }
                                        if (!('sysGridPwr' in payload[channel]['sysEnergyStream'][i]) && adapter.specialPoweroceanZero) {
                                            let haupd;
                                            haupd = await compareUpdate(
                                                adapter,
                                                stateDictObj,
                                            ...
                                            adapter.log.debug(
                                                `${topic} JTS1_ENERGY_STREAM_REPORT` +
                                                    ` artificial set sysGridPwr = 0 from EnergyStreamDetail`,
                                            );
                                            if (Object.keys(haupd).length !== 0) {
                                                haUpdate.push(haupd);
                                            }
                                        }
                                        if (!('bpPwr' in payload[channel]['sysEnergyStream'][i] && adapter.specialPoweroceanZero)) {
                                            let haupd;
                                            haupd = await compareUpdate(
                                                adapter,
                                                stateDictObj,
                                            ...
                                            adapter.log.debug(
                                                `${topic} JTS1_ENERGY_STREAM_REPORT` +
                                                    ` artificial set bpPwr = 0 from EnergyStreamDetail`,
                                            );
                                            if (Object.keys(haupd).length !== 0) {
                                                haUpdate.push(haupd);
                                            }
                            ...
                            case 'HeatingRodEnergyStreamShow':
                                for (let i = 0; i < payload[channel]['hrEnergyStream'].length; i++) {
                                    ...
                                    if (!('hrPwr' in payload[channel]['hrEnergyStream'][i]) && adapter.specialPoweroceanZero) {
                                        let haupd;
                                        haupd = await compareUpdate(
                                            adapter,
                                            stateDictObj,
                                        ...
                                        adapter.log.debug(
                                            `${topic} HeatingRodEnergyStreamShow` + ` artificial set hrPwr = 0`,
                                        );
                                        if (Object.keys(haupd).length !== 0) {
                                            haUpdate.push(haupd);
                                        }
                                    }
                                    if (!('fromPv' in payload[channel]['hrEnergyStream'][i]) && adapter.specialPoweroceanZero) {
                                        let haupd;
                                        haupd = await compareUpdate(
                                            adapter,
                                            stateDictObj,
                                        ...
                                        adapter.log.debug(
                                            `${topic} HeatingRodEnergyStreamShow` + ` artificial set fromPv = 0`,
                                        );
                                        if (Object.keys(haupd).length !== 0) {
                                            haUpdate.push(haupd);
                                        }
                                    }
                                    if (!('fromBat' in payload[channel]['hrEnergyStream'][i])&& adapter.specialPoweroceanZero) {
                                        let haupd;
                                        haupd = await compareUpdate(
                                            adapter,
                                            stateDictObj,
                                        ...
                                        adapter.log.debug(
                                            `${topic} HeatingRodEnergyStreamShow` + ` artificial set fromBat = 0`,
                                        );
                                        if (Object.keys(haupd).length !== 0) {
                                            haUpdate.push(haupd);
                                        }
                                    }
                                    if (!('fromGrid' in payload[channel]['hrEnergyStream'][i])&& adapter.specialPoweroceanZero) {
                                        let haupd;
                                        haupd = await compareUpdate(
                                            adapter,
                                            stateDictObj,
                                        ...
                                        adapter.log.debug(
                                            `${topic} HeatingRodEnergyStreamShow` + ` artificial set fromGrid = 0`,
                                        );
                                        if (Object.keys(haupd).length !== 0) {
                                            haUpdate.push(haupd);
                                        }
                            ...
                            case 'JTS1_BP_STA_REPORT':
                                ...
                                for (let i = 0; i < payload[channel]['bpSta'].length; i++) {
                                    let nr;
                                    nr = payload[channel]['bpSta'][i]['bpDsrc'];
                                ...
                                for (let state in payload[channel]) {
                                    switch (state) {
                                        ...
                                        case 'pcsLoadInfo':
                                            ...
                                            break;
                                        // verschieben von pcsLoadInfo da auch hier ein array möglich
                                        //
                                        case 'meterHeartBeat':
                                            if (
                                                ...
                                                // state = 'meterHeartBeat' nicht doppelt?
                                                for (
                                                    let i = 0;
                                                    i < payload[channel][state]['meterHeartBeat'].length;
                                                    i++
                                                ) {
...
async function prepareProtoCmd(adapter, serial, streamType, state, value, cmd, log) {
    ...
    if (log) {
        adapter.log.debug(`[PROTOBUF encode] ${state} ${value} send poweroceanplus ${JSON.stringify(muster)}`);
    }
    const root = protobuf.parse(protoSource).root;
    const SetMessage = root.lookupType('setMessage');
    const message = SetMessage.create(muster);
...
message JTS1_EMS_HEARTBEAT {
	...
	optional LoadInfo pcsLoadInfo = 15; //flatten repeated
...
message EnergyStreamDetail {
  ...
  message EnergyStreamItem {
	optional float sys_load_pwr = 1; //renamed from load_pwr tp match JTS1_ENERGY_STREAM_REPORT
	optional float sys_grid_pwr = 2; //renamed from grid_pwr tp match JTS1_ENERGY_STREAM_REPORT
