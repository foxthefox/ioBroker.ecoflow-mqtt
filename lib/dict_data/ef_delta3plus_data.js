...
const deviceStatesDict = {
    delta3plus: {
        ...
        DisplayPropertyUpload: {
            ...
            bypassOutDisable: { entity: 'string' }, //switch
...
const deviceCmd = {
    delta3plus: {
        DisplayPropertyUpload: {
            ...
            backupReverseSoc: { msg: { dest: 2, cmdFunc: 254, cmdId: 17, dataLen: 3 } },
        },
        setDp3: {
...
async function prepareProtoCmd(adapter, serial, streamType, state, value, cmd, log) {
    ...
    switch (state) {
        ...
        case 'backupReverseSoc': {
            ...
            if (value < cmsMinDsgSoc.val + 5) {
                value = cmsMinDsgSoc.val + 5;
            }
            muster['header']['pdata'] = {};
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-expect-error
        ...
        case 'cmsMinDsgSoc': {
            muster['header']['pdata'] = {};
            const backupReverseSoc = await adapter
                .getStateAsync(`${serial}.DisplayPropertyUpload.backupReverseSoc`)
                .catch(e => {
                    adapter.log.warn(`did not get ${serial}.DisplayPropertyUpload.backupReverseSoc ->${e}`);
                });
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-expect-error
            muster['header']['pdata'][state] = parseInt(value); //int
            ...
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-expect-error
            if (backupReverseSoc.val < parseInt(value) + 5) {
                // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                // @ts-expect-error
                muster['header']['pdata']['backupReverseSoc'] = parseInt(value) + 5;
