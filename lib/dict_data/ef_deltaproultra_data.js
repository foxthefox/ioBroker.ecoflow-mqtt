...
const protoMsg = {
    cmdId: {
        ...
        72: { cmdFunc: { 2: 'acDischarge_pack' } },
        68: { cmdFunc: { 2: 'dcoutput_pack' } },
        65: { cmdFunc: { 2: 'rename_pack' } },
        //34: { cmdFunc: { 2: 'EnergyStreamDetail' } },
        //33: { cmdFunc: { 2: 'EnergyStreamReport' } },
        //28: { cmdFunc: { 3: 'nodata_src6' } }, //BMS_HEARTBEAT_PUSH_CMD_ID?
...
const deltaproultra = {
	cmdFunc: {
		1: { cmdId: { 86: "town_pack", 87: "areaTown_pack" } },
		2: {
			cmdId: {
				1: "AppShowHeartbeatReport",
				2: "BackendRecordHeartbeatReport",
				3: "AppParaHeartbeatReport",
				4: "BPInfoReport",
				65: "rename_pack",
				68: "dcoutput_pack",
				72: "acDischarge_pack",
				73: "acChargeSpeed_pack",
				81: "deviceTimeout_pack",
				82: "lcdTimeout_pack",
				83: "acTimeout_pack",
				84: "dcTimeout_pack",
				87: "chaLim_pack",
				88: "dischaLim_pack",
				89: "batPreCondOn_pack",
				93: "AcOftenOpen_pack",
				94: "backup_pack",
				103: "lab_pack",
			},
		},
		3: { cmdId: { 28: "BMS_HEARTBEAT_PUSH_CMD_ID" } },
		53: { cmdId: { 117: "lte4g_pack" } },
		254: { cmdId: { 21: "DisplayPropertyUpload" } },
	},
},
*/

const msgNameObj = {
    AppShowHeartbeatReport: { cmdId: 1, cmdFunc: 2 },
    BackendRecordHeartbeatReport: { cmdId: 2, cmdFunc: 2 },
    AppParaHeartbeatReport: { cmdId: 3, cmdFunc: 2 },
    BPInfoReport: { cmdId: 4, cmdFunc: 2 },
                    ...
                    switch (channel) {
                        ...
                        case 'BPInfoReport':
                            ...
                            for (let i = 0; i < payload[channel]['bpInfo'].length; i++) {
                                ...
                                let name = 'BPInfo';
                                switch (payload[channel]['bpInfo'][i]['bpNo']) {
                                    case '1':
                                    case 1:
                                        name = 'BPInfo';
                                        break;
                        ...
                        case 'lte4g_pack':
                        case 'backup_pack':
                        case 'AcOftenOpen_pack':
                        case 'batPreCondOn_pack':
                        case 'dischaLim_pack':
                        case 'chaLim_pack':
                        case 'dcTimeout_pack':
                        case 'acTimeout_pack':
                        case 'lcdTimeout_pack':
                        case 'deviceTimeout_pack':
                        case 'acChargeSpeed_pack':
                        case 'acDischarge_pack':
                        case 'rename_pack':
                            for (let state in payload[channel]) {
                                let val;
                                let haupd;
...
async function prepareProtoCmd(adapter, serial, streamType, state, value, cmd, log) {
    ...
    if (log) {
        adapter.log.debug(`[PROTOBUF encode] ${state} ${value} send deltaproultra ${JSON.stringify(muster)}`);
    }
    const root = protobuf.parse(protoSource).root;
    const SetMessage = root.lookupType('setMessage');
    const message = SetMessage.create(muster);
...
message deviceTimeout_pack {
	optional int32 powerStandbyMins = 1;
}
message lcdTimeout_pack {
	optional int32 screenStandbySec = 1;
}    
message dcTimeout_pack {
	optional int32 dcStandbyMins = 1;
} 
message acTimeout_pack {
	optional int32 acStandbyMins = 1;
} 
message chaLim_pack {
	optional int32 chgMaxSoc = 1;
}
message dischaLim_pack {
	optional int32 dsgMinSoc = 1; 
}
message acChargeSpeed_pack {
	optional int32 chg_5p8SetWatts = 1; // acChargeSpeedPower
	optional int32 chgC20SetWatts = 2; // acChargeSpeedAC
	// optional uint32 chg_pause_flag = 3;
}
message backup_pack {
	optional int32 energyManageEnable = 1; 
	optional int32 backup_ratio = 2;
}
message lte4g_pack {
	optional int32 wireless_4gOn = 1;
}
message AcOftenOpen_pack {
	optional int32 ac_often_open_flg = 1;
	optional int32 ac_often_open_min_soc = 2;
}

message acDischarge_pack {
...
message batPreCondOn_pack {
	optional int32 batPreCondOn = 1; //en_bp_heat
}
...
message setValue {
    ...
    message sentAppShowHeartbeatReport {
        setHeader1 header = 1;
    }
    message sentBackendRecordHeartbeatReport {
        setHeader2 header = 1;
    }
    ...
    message sentAppParaHeartbeatReport {
        setHeader4 header = 1;
    } 

    message sentBPInfoReport {
        setHeader5 header = 1;
    } 

 message setHeader5 {
    BPInfoReport pdata = 1;
    int32 src = 2;
    int32 dest = 3;
    int32 d_src = 4;
    int32 d_dest = 5;
 ...
 message setHeader4 {
    AppParaHeartbeatReport pdata = 1;
    int32 src = 2;
    int32 dest = 3;
    int32 d_src = 4;
    int32 d_dest = 5;
 ...
 message setHeader2 {
    BackendRecordHeartbeatReport pdata = 1;
    int32 src = 2;
    int32 dest = 3;
    int32 d_src = 4;
    int32 d_dest = 5;
...
message setHeader1 {
    AppShowHeartbeatReport pdata = 1;
    int32 src = 2;
    int32 dest = 3;
    int32 d_src = 4;
    int32 d_dest = 5;
