...
function convertProtoToObject(filename) {
    var string = require('node:fs').readFileSync('./'.concat(filename), 'utf8');
...
function createArreayOfFieldnumber(filename) {
    var string2 = require('node:fs').readFileSync('./'.concat(filename), 'utf8');
...
/**
 * @param {string | any[]} array - array
 * @param {{ [x: string]: { [x: string]: any; }; }} protoObj - proto def
 */
...
/**
 * @param {{ [x: string]: { [x: string]: any; }; }} obj - obj
 * @param {{ [x: string]: { [x: string]: any; }; }} names - names
 */
...
/**
 * @param {string | number} device -device
 * @param {{ [x: string]: { [x: string]: { [x: string]: { [x: string]: any; }; }; }; }} baseItem - baseItem
 */
 ...
 */
function createMsgObjFromString(string, type) {
    var textlines = string.split(/\r?\n/);
    let message = {};
    for (let i = 0; i < textlines.length; i++) {
        var linesplit = textlines[i].split(': ');
        if (type === 'set' && linesplit[1].includes('set')) {
            message['msg'.concat(String(i))] = linesplit[2];
        } else if (type === 'get' && linesplit[1].includes('get')) {
            message['msg'.concat(String(i))] = linesplit[2];
        } else if (type === 'update' && linesplit[1].includes('update')) {
            message['msg'.concat(String(i))] = linesplit[2];
        } else if (type === 'all') {
            message['msg'.concat(String(i))] = linesplit[2];
        }
