...
const alternatorCmd = {
	alternator: {
		alternatorHeartbeat: {
			...
			cableLength608: { msg: { dest: 20, cmdFunc: 254, cmdId: 17, dataLen: 6 } }, // #?
...
function prepareProtoCmd(adapter, serial, streamType, state, value, cmd, log) {
	if (state === 'latestQuotas') {
		...
		if (state === 'startStop') {
			muster['header']['pdata'] = {};
			muster['header']['pdata']['startStopCmd'] = value ? 1 : 0;
			muster['header']['dataLen'] = cmd['msg']['dataLen'] || 3;
		} else if (state === 'permanentWatts') {
			muster['header']['pdata'] = {};
			muster['header']['pdata']['permanentWattsCmd'] = value; //float
			muster['header']['dataLen'] = cmd['msg']['dataLen'] || 6;
		} else if (state === 'operationMode') {
			muster['header']['pdata'] = {};
			muster['header']['pdata']['operationModeCmd'] = value; //int
			muster['header']['dataLen'] = cmd['msg']['dataLen'] || 3;
		} else if (state === 'startVoltage') {
			muster['header']['pdata'] = {};
			muster['header']['pdata']['startVoltageCmd'] = parseInt(String(value * 10)); //11V = 111
